...
class EntryPoint:
    ...
    def __init__(self, argv: list[str] = None, initializer: Callable[[], None] = None):
        from src.classifier.config.state import Flags, System
        ...
        # Only call prelude when the selected main task defines its own prelude.
        # Some tasks inherit an interface placeholder from Main; that placeholder
        # is not a real implementation and should not be called.
        prelude = cls.__dict__.get("prelude", None)
    ...
    def run(self, reproducible: Callable = None):
        from src.classifier.config import setting as cfg
        from src.classifier.config.main.analyze import run_analyzer
