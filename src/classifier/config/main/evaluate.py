...
from src.classifier.task.model import ModelRunner

from ..setting import ResultKey
from ._utils import SelectDevice, progress_advance

if TYPE_CHECKING:
    from src.classifier.nn.dataset.evaluation import EvalDatasetLike
    ...
    def run(self, parser: EntryPoint):
        ...
        with Progress.new(
            ...
            if self.opts.max_evaluators == 1:
                ...
                # Avoid ProcessPoolExecutor/spawn hangs when evaluating one model.
                runner = _eval_model(self.device, dataset)
                results = []
                for evaluator in evaluators:
                    results.append(runner(evaluator))
