...
class _roc_select_sig:
    ...
    def _select(self, batch: BatchType):
        ...
        indices = [i for i in MultiClass.indices(*_BKG, self.sig) if i is not None]
        return torch.isin(label, label.new_tensor(indices))
...
class _roc_select_ggF(_roc_select_sig):
    ...
    def _select(self, batch: BatchType):
        ...
        label = batch[Input.label]
        bkg_indices = [i for i in MultiClass.indices(*self.bkg) if i is not None]
        return torch.isin(label, label.new_tensor(bkg_indices)) | (
...
class _roc_cat_by_largest:
    ...
    def __call__(self, batch):
        ...
        bkg_indices = [i for i in MultiClass.indices(*_BKG) if i is not None]
        bkg = torch.isin(y_true, y_true.new_tensor(bkg_indices))
        select = sig | bkg
...
class Eval(HCREval):
    ...
    def output_definition(batch: BatchType):
        ...
        # Some tiny SvB tests do not produce a ttbar output class.
        # When p_ttbar is missing, use zeros so p_bkg reduces to p_multijet.
        p_ttbar = batch.get("p_ttbar", 0 * batch["p_multijet"])
        ...
        output = {
            ...
            "p_multijet": ...,
            "p_ttbar": p_ttbar,
            "p_bkg": batch["p_multijet"] + p_ttbar,
        }
        for sig in ("ZZ", "ZH", "ggF"):
            sig = f"p_{sig}"
            if sig in batch:
