...
def _parquet_to_pandas(data):
    ...
    for field in data.fields:
        ...
        try:
            ...
            pass
        columns[field] = pd.Series(akpd.AwkwardExtensionArray(arr))
    ...
    **options : dict, optional
    ...
    def _open_with_retry(self, path, retries=5, delay=5):
        ...
        for attempt in range(retries):
            try:
                return uproot.open(path, **self._open_options)
            except (FileNotFoundError, OSError) as e:
                last_exc = e
                if attempt < retries - 1:
                    logging.warning(
                        f"Failed to open {path} (attempt {attempt + 1}/{retries}), "
        ...
        **options : dict, optional
        ...
        if path.endswith(".parquet"):
            ...
            if library == "ak":
                ...
                out = _parquet_to_pandas(data)
                out.reset_index(drop=True, inplace=True)
            elif library == "np":
                out = {field: ak.to_numpy(data[field]) for field in data.fields}
            else:
